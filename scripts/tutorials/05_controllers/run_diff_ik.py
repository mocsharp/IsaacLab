...
class ControllerData:
    def __init__(self):
        ...
        self.incoming_messages = collections.deque(maxlen=60)
    ...
    def get_incoming_message(self) ->  tuple[Optional[InputCommand], Optional[int]]:
        if len(self.incoming_messages) == 0:
            return (None, None)
        return self.incoming_messages.popleft()
    
    def add_incoming_message(self, message: InputCommand):
        """Store HID event timestamps and message ID.
        
        Args:
            message: The input command message
        """

        self.incoming_messages.append((message, time.time_ns()))

        # record all unique message IDs
        self.unique_message_ids.add(message.message_id)
    ...
    def update_from_joysticks(self, dt: float, data: Optional[InputCommand], receive_time: int):
        ...
        if data is not None:
                        ...
                        case 1:  # buttons
                            match data.number:
                                case 0 | 1 | 2 | 3:  # A/B/X/Y - Cycle through robots
                                    if data.value != 0:
                                        robot_index = (self.robot_index + 1) % args_cli.num_envs
                                        self.set_robot_index(robot_index)
                                ...
                                case 5:  # left - controls wrist_3 (negative direction)
                                    if data.value != 0:  # Button pressed
                                        self.set_joystick_value("wrist_3", -1.0)
                                        message_handled = True
                                        # logger.info(f"Set wrist_3 joystick: -1.0 (left button)")
                                    else:  # Button released
                                        self.set_joystick_value("wrist_3", 0.0)
                                        message_handled = True
                            ...
                            match data.number:
                                ...
                                case 6 | 7: # ZR/ZL - stop wrist_2 movement
                                    if normalized_value == -1.0:
                                        self.set_joystick_value("wrist_2", 0.0)
                                        message_handled = True
            ...
            camera_info.capture_timestamp = data.capture_timestamp
            camera_info.hid_publish_timestamp = data.publish_timestamp
            camera_info.receive_timestamp = receive_time
            camera_info.message_id = data.message_id
            # Reset sim_step_start_timestamp for this message template
            camera_info.camera_update_time = idl.int64(0) 
        else:
            camera_info = CameraInfo()
            camera_info.message_id = idl.uint64(0)
...
class RobotCameraPublisher(Publisher):
    ...
    def produce(self, controller_data: ControllerData, camera_data: Any, camera_event: Any):
        ...
        if current_time - last_stats_time >= stats_interval:
            ...
            input_fps = total_input_commands_received / elapsed_time if elapsed_time > 0 else 0
            camera_fps = total_camera_info_messages_sent / elapsed_time if elapsed_time > 0 else 0
            
            logger.info("=== Python Application Message Statistics ===")
            logger.info(f"Total InputCommand messages received: {total_input_commands_received} ({input_fps:.2f} fps)")
            logger.info(f"Total CameraInfo messages sent: {total_camera_info_messages_sent} ({camera_fps:.2f} fps)")
            ...
            logger.info(f"Queued messages: {len(controller_data.incoming_messages)}")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, controller_data: ControllerData, robot_camera_publisher: RobotCameraPublisher):
    ...
    while simulation_app.is_running():
        ...
        event, receive_time = controller_data.get_incoming_message()

        # # Update joint positions based on joystick state
        camera_info = controller_data.update_from_joysticks(sim_dt, event, receive_time)
        ...
        if camera_info is not None:
            camera_info.camera_update_time = int(time.time_ns())
        
        # Access camera data from the selected robot
        rgb_data = camera.data.output.get("rgb")
        
        if rgb_data is not None:
            video_queue.append((rgb_data[controller_data.robot_index], camera_info))
        ...
        if elapsed_time >= eps_report_interval:
            ...
            logger.info(f"Simulation loop EPS: {eps:.2f}")
...
def hid_callback(topic: str, controller_data: ControllerData, data: InputCommand):
    ...
    controller_data.add_incoming_message(data)
